...
public class UserServiceJPA implements UserService {
	...
	/**
	 * Database extension needed for password hashing extension will be set once as
	...
	/**
	 * Login Method for user login, method will select user and hashed password from
	 * database
	 * 
	 * @param login
	 *            - user login to be selected
	 * @param password
	 *            - user password to be selected
	...
	public ForumUser login(String login, String password) {
		try {
			return (ForumUser) entityManager.createQuery(
					...
					// "SELECT fu FROM ForumUser fu WHERE fu.login =:login AND fu.password
					// =:password")
	 ...
	 * 
	 * @param login
	 *            - user login to be used for selecting specific profile picture
	...
	/**
	 * isAdmin method to select only users with admin rights(needed for specific
	 * functions)
	 * 
	 * @param login
	 *            - user login to be used for selecting user rights from database
	 ...
	 * 
	 * @param login
	 *            - selecting specific user by login
	 ...
	 * 
	 * @param login
	 *            - to change/update specific user
	 * @param value
	 *            - to set specific value (admin/user)
	 */
	 ...
	 * 
	 * @param login
	 *            - to change profile picture for specific login
	 * @param pic
	...
	/**
	 * Method for email change, email can be changed by admin from admin zone or by
	 * user from user profile
	 * 
	 * @param login
	 *            - username of wich user mail have to be edited
	 * @param email
	 *            - new email address
	 */
	public void emailChange(String login, String email) {
		if (!email.isEmpty()) {
			// entityManager.createQuery("UPDATE ForumUser fu SET fu.email =:email WHERE
			// fu.login = :login")
			// .setParameter("email", email).setParameter("login", login).executeUpdate();
			ForumUser fu = (ForumUser) entityManager.createQuery("SELECT fu from ForumUser fu WHERE fu.login = :login")
					.setParameter("login", login).getSingleResult();
			fu.setEmail(email);
	...
	/**
	 * Method for password change, admin can change password for all users or users
	 * can update their password from user profile page.
	 * 
	 * @param login
	 *            - of which user password have to be changed
	 * @param password
	 *            - new password
	 */
	...
	/**
	 * Method for password validation on back-end side. At least one Number At least
	 * one Lower Case letter At least one Upper Case letter Minimal length 6
	 * characters Maximal length 255 characters
	 * 
	 * @param password
	 *            to be set
